...
void vulkan_image_create(vulkan_context* context,
    ...
    image_create_info.extent.depth = 1; // TODO(frama): Support configurable depth
    image_create_info.mipLevels = 4; // TODO(frama): Support mip mapping
    image_create_info.arrayLayers = 1; // TODO(frama): Support number of layers in image
    image_create_info.format = format;
    image_create_info.tiling = tiling;
    image_create_info.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
    image_create_info.usage = usage;
    image_create_info.samples = VK_SAMPLE_COUNT_1_BIT; // TODO(frama): Configurable sample
    image_create_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // TODO(frama): Configurable sharing mode
    ...
    // Bind the memory.
    VK_CHECK(vkBindImageMemory(context->device.logical_device, out_image->handle, out_image->memory, 0)); // TODO(frama): Configurable memory offset.
...
void vulkan_image_view_create(vulkan_context* context,
    ...
    view_create_info.viewType = VK_IMAGE_VIEW_TYPE_2D; // TODO(frama): Make configurable.
    view_create_info.format = format;
    view_create_info.subresourceRange.aspectMask = aspect_flags;
    
    // TODO(frama): Make configurable.
