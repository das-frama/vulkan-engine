...
b8 vulkan_swapchain_acquire_next_image_index(vulkan_context* context,
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        ...
        FATAL("Failed to acuire swapchain image!");
        return false;
    }
...
void vulkan_swapchain_present(vulkan_context* context,
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        // Swapchain is out of date, suboptimal or a framebuffer resize has occured.
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    if (!swapchain->images) {
        swapchain->images = (VkImage*)memory_allocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views) {
...
void destroy(vulkan_context* context, vulkan_swapchain* swapchain) {
    ...
    // Only destroy the view, not the images.
    for (u32 i = 0; i < swapchain->image_count; ++i) {
