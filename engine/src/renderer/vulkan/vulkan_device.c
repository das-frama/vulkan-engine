...
b8 vulkan_device_create(vulkan_context* context) {
    ...
    // NOTE(frama): Do not create additional queues for shared instances.
    ...
    for (u32 i = 0; i < index_count; ++i) {
        ...
        // TODO(frama): Enable this for a future enhacements.
    ...
    // TODO(frama): Should be config driven.
    ...
    // Depreceted and ignored by Vulkan.
...
b8 select_physical_device(vulkan_context* context) {
    ...
    for (u32 i = 0; i < physical_device_count; ++i) {
        ...
        // NOTE(frama): Enable this if compute will be required.
        ...
        if (result) {
            ...
            // NOTE(frama): set compute index here if needed.
    ...
    if (!context->device.physical_device) {
        ERROR("No physical device were found which meet the requirements.");
        return false;
    }
...
b8 physical_device_meets_requirements(VkPhysicalDevice device, 
    ...
    // Look at each queue and see what queues if supports.
    ...
    for (u32 i = 0; i < queue_family_count; ++i) {
        ...
        if (queue_families[i].queueFlags & VK_QUEUE_TRANSFER_BIT) {
            ...
            // likleyhood that it is a dedicated transfer queue.
    ...
    if ((!requirements->graphics || (requirements->graphics && out_queue_info->graphics_family_index != -1)) &&
        ...
        if (out_swapchain_support->format_count < 1 || out_swapchain_support->present_mode_count < 1) {
            if (out_swapchain_support->formats) {
                memory_free(out_swapchain_support->formats,
                            sizeof(VkSurfaceFormatKHR) * out_swapchain_support->format_count, 
                            MEMORY_TAG_RENDERER);
            }
            if (out_swapchain_support->present_modes) {
                memory_free(out_swapchain_support->present_modes,
